...
/*
Modified since I don't need printf and can't link to any kind of library...
Macros are fine though.

stdarg.h have macros to handle variable argument lists, which is needed.
Don't need stdlib.h
Don't need usart library, cant be arsed merging cmsis with this one.
-irving
*/
...
int sprintf_(unsigned char *buffer, const char *format, ...)
{
